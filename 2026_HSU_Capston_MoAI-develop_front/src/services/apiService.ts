...
function wrappedArray(value: unknown, keys: string[]): unknown[] {
...
function normalizeResource(value: unknown): ResourceItem | null {
  ...
  const url = stringValue(obj.url)
    || stringValue(obj.fileUrl)
    || stringValue(obj.resourceUrl)
    || stringValue(obj.downloadUrl)
  ...
  const type = stringValue(obj.type)
    || stringValue(obj.resourceType)
    || stringValue(obj.fileType)
    || 'pdf'
...
function normalizeWeekDetail(raw: unknown, fallbackWeekId: string): CurriculumWeekDetail {
...
function normalizeWeekSummary(value: unknown): CurriculumWeekSummary | null {
  const obj = asRecord(value)
  if (!obj) return null
...
function normalizeRecommendedVideo(value: unknown): RecommendedVideo | null {
  const obj = asRecord(value)
  if (!obj) return null
...
/**
 * deleteLearningRoom  —  DELETE /api/learning-rooms/{roomId}  (JWT 필요)
 */
